...
def auto_play_from_api(base_url: str, agents: List[str]):
    ...
    t = threading.Thread(
        target=console_listener, args=(queue, known_hashes, base_url), daemon=True
    )
    t.start()
    ...
    threading.Thread(
        ...
        args=("enp4s0", "14.103.255.172", 11003),
        kwargs={"out_queue": queue, "stop_event": stop_evt},
    ).start()
    while True:
        while True:
            if play == False:
        ...
        try:
            ...
            print(
                f"[ERROR] download/parse failed: {name} ({hash_}) -> {e}",
                file=sys.stderr,
            )
            continue
            ...
            if ti.tm_hour == 4:
                if ti.tm_min == 59:
                    ...
                    for agent_url in agents:
                        try:
                            r = requests.post(
                                agent_url.rstrip("/") + "/esc", json={}, timeout=5
                            )
                            r.raise_for_status()
                        except Exception as e:
                            print(
                                f"[WARN] notify agent {agent_url} failed: {e}",
                                file=sys.stderr,
                            )
