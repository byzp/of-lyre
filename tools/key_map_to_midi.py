    ...
    for octv in base_octaves:
        for off in white_offsets:
            if idx >= len(keys):
                break
            letter = keys[idx]
            midi_note = (octv + 0) * 12 + 0 + off  # C_octv midi = 12*octv, we add offset
...
def build_midi_events(parsed_events, ticks_per_beat=TICKS_PER_BEAT, beats_per_unit=BEATS_PER_UNIT):
...
def scheduled_to_midifile(scheduled, bpm=BPM, ticks_per_beat=TICKS_PER_BEAT, out_filename=OUTPUT_FILE):
