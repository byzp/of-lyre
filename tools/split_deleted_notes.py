...
def parse_args():
    ...
    p.add_argument("--start-tol", type=float, default=0.05, help="起始时间容差（秒），默认 0.05")
    p.add_argument("--dur-tol", type=float, default=0.05, help="时长容差（秒），默认 0.05")
...
def main():
    ...
    deleted_by_inst = find_deleted_notes(full_pm, reduced_notes, start_tol=args.start_tol, dur_tol=args.dur_tol)
    ...
    for inst_idx, deleted_notes in deleted_by_inst.items():
        ...
        new_name = f"deleted_from_{src_inst.name or ('program_' + str(src_inst.program))}"
        del_inst = pretty_midi.Instrument(program=src_inst.program, is_drum=src_inst.is_drum, name=new_name)
        for n in deleted_notes:
            # 复制 note（pitch, start, end, velocity）
            new_note = pretty_midi.Note(velocity=getattr(n, "velocity", 100),
                                       pitch=n.pitch,
                                       start=n.start,
                                       end=n.end)
    ...
    if args.verbose:
        ...
        print(f"共检测到 {sum(len(inst.notes) for inst in full_pm.instruments)} notes（full）")
        print(f"reduced 中 notes 数量: {len(reduced_notes)}")
        print(f"被判定为 deleted 的 notes 总计: {total_deleted}")
        print("为每个有 deleted note 的原始 instrument 新建了一个 track（名称以 deleted_from_ 开头）")
