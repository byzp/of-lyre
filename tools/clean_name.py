...
def main():
    ...
    print(
        f"发现 {len(ops)} 个待重命名文件（目录：{base}，模式：{args.pattern or '*'}，{'递归' if args.recursive else '不递归'}）："
    )
    for src, dst in ops:
        print(f"  {src}  ->  {dst}")
