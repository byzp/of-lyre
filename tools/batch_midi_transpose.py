...
def process_file(path_in: Path, path_out_dir: Path, args):
    ...
    if len(pitches) == 0:
        print(f"[WARN] 文件 {path_in.name} 不包含可统计的 note（或全部为 drum 并被忽略）。跳过统计。")
        return None
    ...
    if stats['total'] > 0:
        if stats['inrange_counts'] is not None:
            within_pct = sum(stats['inrange_counts'].values()) / stats['total']  # 这一值等于 1 - below - above
        else:
            within_pct = 0.0
...
def main():
    parser = argparse.ArgumentParser(description="批量读取 MIDI，移调以最大化 C3-B5 白键数量，统计并按阈值保存。")
    ...
    print("默认阈值说明：min_within_pct (C3-B5 占比 最小), max_below_pct (低于 C3 占比 最大), max_above_pct (高于 B5 占比 最大)")
    print("若需调整阈值，请使用命令行参数 --min_within_pct/--max_below_pct/--max_above_pct")
